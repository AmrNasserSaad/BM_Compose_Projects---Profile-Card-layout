...
fun TrafficCalculator(
    ...
    Column(
        ...
        Button(
            colors = ButtonDefaults.buttonColors(DarkBlue),onClick = {
            val traffic = when (selected) {
                "Feeling Lucky (+0%)" -> 0.0
                "Safe Zone (+5%)" -> 0.05
                else -> 0.10
            }
            navController.navigate("${TRIP_PRICE}/$km/$mins/$traffic")
...
fun TrafficOptionsScreenPreview() {
    TrafficCalculator(rememberNavController(), 20.5f, 12)
}